...
public class Pig extends Sprite implements Disposable, InputProcessor {
    ...
    public Pig(World world, Ellipse ellipse) {
        super(EasyLevel.atlas.findRegion("Block"));
    ...
    public static int getCircleArrayLength() {
        return ControllerLogic.circleArray.size;
    }
    ...
    public TextureRegion getTR(float dt) {
        ...
        switch (currentState) {
            case GOOD:
                region = (TextureRegion) pigGood.getKeyFrame(stateTime, true);
                break;
            case BAD:
                region = (TextureRegion) pigBad.getKeyFrame(stateTime, true);
                break;
            case VERYBAD:
                region = (TextureRegion) pigVeryBad.getKeyFrame(stateTime, true);
                break;
            default:
                region = (TextureRegion) pigDaid.getKeyFrame(stateTime, false);
                break;
