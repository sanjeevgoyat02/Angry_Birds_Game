...
public class HardLevel extends InputAdapter implements Screen, ILevelScreen, Serializable {
    ...
    public HardLevel(AngryBirds game) {
        this.game = game;
        this.level = "Hard"; // Default level name
        resetGame();
    ...
    public void saveState(String filePath) {
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(filePath))) {
            ...
            Gdx.app.log("HardLevel", "Game state saved successfully.");
        } catch (IOException e) {
            Gdx.app.log("HardLevel", "Failed to save game state: " + e.getMessage());
        }
    }

    public static HardLevel loadState(String fileName, AngryBirds game) {
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fileName))) {
            ...
            HardLevel loadedLevel = new HardLevel(game);
            loadedLevel.level = level;
            loadedLevel.score = score;
            loadedLevel.chances = chances;
            loadedLevel.gameOver = gameOver;
            Gdx.app.log("HardLevel", "Game state loaded successfully.");
            return loadedLevel;
        } catch (IOException | ClassNotFoundException e) {
            Gdx.app.log("HardLevel", "Failed to load game state: " + e.getMessage());
            return null;
        }
    }
    ...
    private void initializeHardcodedObjects() {
        // Base layer of blocks
        ControllerLogic.IceboxArray.add(new Ice_Box(world, new Rectangle(580.00F, 60.00F, 30.00F, 160.00F), 1f, 1f)); // Left base block
        ControllerLogic.StoneboxArray.add(new Stone_Box(world, new Rectangle(680.00F, 60.00F, 30.00F, 160.00F), 1f, 1f)); // Middle base block
        ControllerLogic.boxArray.add(new Box(world, new Rectangle(780.00F, 60.00F, 30.00F, 160.00F), 1f, 1f)); // Right base block

        // Horizontal block connecting middle and right base blocks
        ControllerLogic.boxArray.add(new Box(world, new Rectangle(630.00F, 200.00F, 220.00F, 30.00F), 1f, 1f)); // Middle horizontal block

        // Vertical tower on top of left base block
        ControllerLogic.IceboxArray.add(new Ice_Box(world, new Rectangle(580.00F, 220.00F, 30.00F, 80.00F), 1f, 1f)); // First vertical block

        // Add pigs in challenging positions
        ControllerLogic.circleArray.add(new Pig(world, new Ellipse(680.00F, 65.00F, 40.00F, 40.00F)));
        ControllerLogic.LargecircleArray.add(new LargePig(world, new Ellipse(700.00F, 220.00F, 40.00F, 40.00F)));
        ControllerLogic.MediumcircleArray.add(new MediumPig(world, new Ellipse(830.00F, 220.00F, 40.00F, 40.00F)));
        ControllerLogic.circleArray.add(new Pig(world, new Ellipse(630.00F, 20.00F, 40.00F, 40.00F)));
    }
    ...
    public void setPaused(boolean paused) {
        ...
        if (paused) {
            Gdx.app.log("HardLevel", "Game Paused");
        } else {
            Gdx.app.log("HardLevel", "Game Resumed");
        }
    }
    ...
    public boolean keyDown(int keycode) {
        ...
        if (keycode == Input.Keys.S) { // Save game state
            HardLevel currentLevel = this;
            currentLevel.saveState("game_state_hard.ser");
            Gdx.app.log("HardLevel", "Game state saved.");
        }
        else if (keycode == Input.Keys.L) { // Load game state
            HardLevel loadedLevel = HardLevel.loadState("game_state_hard.ser", game);
            if (loadedLevel != null) {
                game.setScreen(loadedLevel); // Switch to the loaded level
                Gdx.app.log("HardLevel", "Game state loaded.");
            } else {
                Gdx.app.log("HardLevel", "No saved state found or failed to load.");
            }
        }
