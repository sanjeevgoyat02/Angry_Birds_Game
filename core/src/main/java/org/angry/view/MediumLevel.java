...
public class MediumLevel extends InputAdapter implements Screen, ILevelScreen, Serializable {
    ...
    public MediumLevel(AngryBirds game) {
        this.game = game;
        this.level = "Medium"; // Default level name
        resetGame();
    ...
    public void saveState(String filePath) {
        try (ObjectOutputStream out = new ObjectOutputStream(new FileOutputStream(filePath))) {
            ...
            Gdx.app.log("MediumLevel", "Game state saved successfully.");
        } catch (IOException e) {
            Gdx.app.log("MediumLevel", "Failed to save game state: " + e.getMessage());
        }
    }

    public static MediumLevel loadState(String fileName, AngryBirds game) {
        try (ObjectInputStream in = new ObjectInputStream(new FileInputStream(fileName))) {
            ...
            MediumLevel loadedLevel = new MediumLevel(game);
            loadedLevel.level = level;
            loadedLevel.score = score;
            loadedLevel.chances = chances;
            loadedLevel.gameOver = gameOver;
            Gdx.app.log("MediumLevel", "Game state loaded successfully.");
            return loadedLevel;
        } catch (IOException | ClassNotFoundException e) {
            Gdx.app.log("MediumLevel", "Failed to load game state: " + e.getMessage());
            return null;
        }
    }
    ...
    private void initializeHardcodedObjects() {
        ...
        ControllerLogic.StoneboxArray.add(new Stone_Box(world, new Rectangle(700.00F, 60.00F, 46.67F, 149.33F), 1f, 1f)); // Middle block
        ControllerLogic.boxArray.add(new Box(world, new Rectangle(820.00F, 60.00F, 30.67F, 160.33F), 1f, 1f));

        // Add a new block above the middle block
        ControllerLogic.boxArray.add(new Box(world, new Rectangle(630.00F, 210.00F, 200.67F, 30.00F), 1f, 1f)); // New block above the middle

        // Add pigs
        ControllerLogic.circleArray.add(new Pig(world, new Ellipse(670.00F, 60.00F, 39.86F, 32.99F))); // Second pig (closer to middle block)
        ControllerLogic.LargecircleArray.add(new LargePig(world, new Ellipse(790.00F, 60.00F, 39.86F, 32.99F))); // Right pig (between middle and right blocks)

        // Add a pig on top of the new block
        ControllerLogic.MediumcircleArray.add(new MediumPig(world, new Ellipse(700.00F, 250.00F, 39.86F, 32.99F))); // Pig above the new block
    }
    ...
    public void setPaused(boolean paused) {
        ...
        if (paused) {
            Gdx.app.log("MediumLevel", "Game Paused");
        } else {
            Gdx.app.log("MediumLevel", "Game Resumed");
        }
    }
    ...
    public boolean keyDown(int keycode) {
        ...
        if (keycode == Input.Keys.S) { // Save game state
            MediumLevel currentLevel = this;
            currentLevel.saveState("game_state_medium.ser");
            Gdx.app.log("MediumLevel", "Game state saved.");
        }
        else if (keycode == Input.Keys.L) { // Load game state
            MediumLevel loadedLevel = MediumLevel.loadState("game_state_medium.ser", game);
            if (loadedLevel != null) {
                game.setScreen(loadedLevel); // Switch to the loaded level
                Gdx.app.log("MediumLevel", "Game state loaded.");
            } else {
                Gdx.app.log("MediumLevel", "No saved state found or failed to load.");
            }
        }
